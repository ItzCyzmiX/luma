...
def on_key_press(key):
    ...
    if key == Luma.Keyboard.F:
        flip_index = (flip_index + 1) % len(flip_modes)
        sprite.flip_mode = flip_modes[flip_index]
    elif key == Luma.Keyboard.C:
        sprite.set_center_point(None if sprite.center_point is not None else (120, 90))
    elif key == Luma.Keyboard.A:
        sprite.alpha = 100 if sprite.alpha == 220 else 220
    elif key == Luma.Keyboard.R:
        sprite.color_mod = (
            (255, 220, 220) if sprite.color_mod != (255, 220, 220) else (220, 255, 220)
        )
    elif key == Luma.Keyboard.K:
        sprite.kill()
